...
func Timer(s *discordgo.Session, ChannelID string, Player1 string) {
	...
	for i = 0; i < len(AllTTTGames); i++ {
		if AllTTTGames[i].Player1 == Player1 {
			exist = true
			break
		}
	}
	if exist && AllTTTGames[i].Player2 == "0" {
...
func TicTacToe(s *discordgo.Session, m *discordgo.MessageCreate) {
	Prefix := config.Prefix
	if strings.HasPrefix(m.Content, Prefix+"t ") {
		message := strings.Replace(m.Content, Prefix+"t ", "", 1)
